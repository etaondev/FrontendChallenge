import { Directive, ElementRef, Renderer2, HostListener, AfterViewInit, Input, OnChanges, SimpleChanges } from '@angular/core';
...
export class FitTextDirective implements AfterViewInit, OnChanges {
  ...
  private fitText() {
    ...
    while (this.element.scrollWidth <= parentWidth && fontSize < 300) {
      fontSize++;
      this.renderer.setStyle(this.element, 'font-size', `${fontSize}px`);
    }
